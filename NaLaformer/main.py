...
from NALA import NALAFORMER_T, NALAFORMER_S, NALAFORMER_B, NALAFORMER_L, NALAFORMER_XT

archs = {
    'NALAFORMER_T': NALAFORMER_T,
    'NALAFORMER_S': NALAFORMER_S,
    'NALAFORMER_B': NALAFORMER_B,
    'NALAFORMER_L': NALAFORMER_L,
    'NALAFORMER_XT': NALAFORMER_XT,
}
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (max_accuracy == test_stats["acc1"]) or (model_ema is not None and max_accuracy == test_stats_ema['acc1']):
            if args.model_ema:
                    utils.save_on_master({
                        'model': model_without_ddp.state_dict(),
                        'optimizer': optimizer.state_dict(),
        ...
        log_stats = {'epoch': epoch, 'n_parameters': n_parameters, **train_f, **test_f}
        if model_ema is not None:
            test_ema_f = {'test_ema_{}'.format(k): v for k, v in test_stats_ema.items()}
            log_stats.update(test_ema_f)
